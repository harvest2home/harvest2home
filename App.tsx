...
const Navigation: React.FC<{ onHome: () => void, onEnroll: () => void, onBuyer: () => void }> = ({ onHome, onEnroll, onBuyer }) => (
  <nav className="bg-[#1b4332] text-white px-6 md:px-12 py-3 hidden md:flex items-center justify-between border-b border-white/10 sticky top-0 z-50 backdrop-blur-lg">
    <div className="flex items-center gap-12 lg:gap-16">
      <div className="flex-shrink-0 transition-all hover:scale-105">
        <MainLogo className="h-32 lg:h-40 w-auto object-contain" onClick={onHome} />
      </div>
      
      <div className="flex gap-6 lg:gap-8 text-[11px] font-black uppercase tracking-[0.2em] text-white/40">
        <button onClick={onHome} className="hover:text-white transition-all">Home</button>
        <button onClick={onBuyer} className="hover:text-white transition-all">Exchange</button>
        <a href="#logistics" className="hover:text-white transition-all">Logistics</a>
        <a href="#support" className="hover:text-white transition-all">Support</a>
      </div>
    </div>
    ...
    <div className="flex gap-5 lg:gap-6 flex-shrink-0 ml-4">
      ...
      <button onClick={onBuyer} className="px-7 lg:px-9 py-3.5 rounded-full bg-[#ff9f1c] text-white text-[10px] font-black uppercase tracking-wider hover:brightness-110 transition-all shadow-lg shadow-orange-500/20 whitespace-nowrap">
...
const LandingPage: React.FC<{ onAction: (role: UserRole) => void }> = ({ onAction }) => {
  ...
  return (
    <div className="min-h-screen bg-white font-outfit relative flex flex-col">
      ...
      {/* Mobile Top Bar */}
      <div className="md:hidden flex items-center justify-center p-6 bg-[#1b4332]">
         <MainLogo className="h-32 w-auto" onClick={() => window.scrollTo(0,0)} />
      </div>
      ...
      <section className="relative h-[85vh] md:h-[90vh] w-full bg-[#1b4332] overflow-hidden flex items-center">
        <div className="absolute inset-0 z-0 bg-[#1b4332]">
          <img 
            ...
            loading="eager"
            onError={(e) => {
               e.currentTarget.src = "https://images.unsplash.com/photo-1542838132-92c53300491e?auto=format&fit=crop&q=80&w=2560";
            }}
          />
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-10 w-full relative z-20">
          <div className="max-w-4xl space-y-6 md:space-y-8 animate-enter">
            ...
            <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-9xl font-black tracking-tighter leading-[0.85] text-white">
              Bharat's <br/>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-5 pt-2">
              <button 
                ...
                className="w-full sm:w-auto px-10 py-5 lg:py-6 rounded-[28px] bg-[#ff9f1c] text-white font-black text-lg lg:text-xl hover:scale-105 transition-all shadow-xl shadow-orange-500/30 italic flex items-center justify-center gap-4 group"
              >
              ...
              <button 
                ...
                className="w-full sm:w-auto px-10 py-5 lg:py-6 rounded-[28px] bg-white text-[#1b4332] font-black text-lg lg:text-xl hover:bg-stone-50 transition-all shadow-xl shadow-black/5 border-2 border-stone-100 flex items-center justify-center"
              >
                Bulk Buyer Access
              </button>
            </div>
      ...
      {/* Features */}
      <section className="max-w-7xl mx-auto px-6 md:px-10 py-24 grid lg:grid-cols-12 gap-12 xl:gap-20">
        <div className="lg:col-span-7 space-y-12">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {[
              ...
              <div key={i} className={`p-8 md:p-10 rounded-[40px] md:rounded-[48px] border ${f.border} ${f.bg} hover-lift shadow-sm flex flex-col items-center text-center group transition-all duration-500`}>
                <div className="mb-6 p-7 bg-white/80 backdrop-blur-sm rounded-[24px] md:rounded-[28px] shadow-sm group-hover:shadow-md transition-all">{f.icon}</div>
        ...
        <div className="lg:col-span-5">
          <div className="bg-white rounded-[40px] md:rounded-[56px] border border-stone-100 shadow-[0_30px_80px_-15px_rgba(0,0,0,0.08)] overflow-hidden lg:sticky lg:top-32">
            <div className="p-7 bg-[#1b4332] text-white">
              ...
              <p className="text-white/30 text-[8px] font-bold uppercase tracking-[0.2em]">Direct Rates</p>
            </div>
      ...
      <footer className="bg-[#0d1b14] text-white/70 pt-24 pb-16 px-6 md:px-12 lg:px-24 border-t border-white/5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 lg:gap-8 mb-24">
            <div className="space-y-8">
              <div className="inline-block transition-all hover:scale-105">
                <FooterLogo className="h-40 w-auto object-contain pointer-events-none" />
              </div>
              <p className="text-lg leading-relaxed font-medium text-white/50 max-w-xs">
                Direct agricultural exchange connecting Bharat's heartland producers to global institutional buyers.
              </p>
            </div>
            ...
            <div className="space-y-8">
              ...
              <nav className="flex flex-col gap-5 text-[15px] font-bold">
                <button onClick={() => setModalContent(LEGAL_CONTENT.terms)} className="text-left hover:text-[#ff9f1c] transition-colors w-fit">Market Protocol</button>
                <button onClick={() => setModalContent(LEGAL_CONTENT.shipping)} className="text-left hover:text-[#ff9f1c] transition-colors w-fit">Supply Chain</button>
              </nav>
            </div>

            <div className="space-y-8">
              <h4 className="text-white font-black uppercase text-[11px] tracking-[0.4em]">Governance</h4>
              <nav className="flex flex-col gap-5 text-[15px] font-bold">
                <button onClick={() => setModalContent(LEGAL_CONTENT.terms)} className="text-left hover:text-[#ff9f1c] transition-colors w-fit">Trade Rules</button>
                <button onClick={() => setModalContent(LEGAL_CONTENT.privacy)} className="text-left hover:text-[#ff9f1c] transition-colors w-fit">KYC Shield</button>
              </nav>
            </div>
        ...
        {modalContent && (
          <div className="fixed inset-0 z-[100] bg-[#0d1b14]/95 backdrop-blur-xl flex items-center justify-center p-6 sm:p-12" onClick={() => setModalContent(null)}>
            <div className="bg-white p-8 md:p-20 rounded-[40px] md:rounded-[80px] max-w-3xl w-full animate-enter text-[#1b4332] shadow-[0_60px_120px_-20px_rgba(0,0,0,0.5)] border border-white/20" onClick={e => e.stopPropagation()}>
              <h3 className="text-3xl md:text-5xl font-black mb-10 tracking-tighter italic uppercase border-b-8 border-[#ff9f1c] pb-4 inline-block">Direct Protocol</h3>
              <p className="text-xl md:text-2xl font-medium leading-[1.6] text-stone-600 mb-12">{modalContent}</p>
              <button onClick={() => setModalContent(null)} className="w-full py-7 bg-[#1b4332] text-white font-black rounded-[24px] md:rounded-[40px] text-xl md:text-2xl hover:brightness-110 active:scale-95 transition-all">ACKNOWLEDGE</button>
            </div>
          </div>
        )}
      </footer>
...
const RegistrationFlow: React.FC<{ role: UserRole, onBack: () => void }> = ({ role, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-[#fcfbf9] flex flex-col items-center justify-center p-6 sm:p-12 font-outfit relative overflow-x-hidden">
      <div className="mb-10"><MainLogo className="h-40 w-auto" onClick={onBack} /></div>
      
      <div className="w-full max-w-2xl bg-white p-8 sm:p-16 md:p-24 rounded-[40px] sm:rounded-[80px] shadow-[0_40px_100px_-20px_rgba(27,67,50,0.15)] border border-stone-100 relative z-10 animate-enter overflow-hidden">
        {step === 1 && (
          <div className="space-y-8 sm:space-y-12">
            <div className="text-center"><h2 className="text-4xl sm:text-6xl font-black text-[#1b4332] mb-4 italic">Secure Entry</h2><p className="text-stone-400 font-black uppercase text-[10px] sm:text-[12px] tracking-[0.5em]">{role} PORTAL</p></div>
            <div className="space-y-8">
              <input type="tel" placeholder="Mobile Number" className="w-full px-8 py-6 sm:py-8 bg-stone-50 rounded-[24px] sm:rounded-[40px] text-2xl sm:text-4xl font-black outline-none border-2 border-transparent focus:border-[#1b4332] shadow-inner" value={phone} onChange={e => setPhone(e.target.value)} />
              <button onClick={() => setStep(2)} className="w-full py-6 sm:py-8 bg-[#1b4332] text-white rounded-[24px] sm:rounded-[40px] font-black text-xl sm:text-3xl shadow-xl shadow-[#1b4332]/20">Request OTP</button>
            </div>
          </div>
        )}
        {step === 2 && (
          <div className="space-y-12 sm:space-y-16 text-center">
            <h2 className="text-4xl sm:text-6xl font-black text-[#1b4332] italic">Identity Verification</h2>
            <div className="flex gap-2 sm:gap-4 justify-center">
              {otp.map((d, i) => (
                <input key={i} maxLength={1} className="w-10 h-20 sm:w-20 sm:h-32 bg-stone-50 rounded-[12px] sm:rounded-[24px] text-center text-3xl sm:text-6xl font-black focus:border-[#ff9f1c] outline-none shadow-sm transition-all" value={d} onChange={e => {
            ...
            <button onClick={() => setStep(3)} className="w-full py-6 sm:py-8 bg-[#ff9f1c] text-white rounded-[24px] sm:rounded-[40px] font-black text-xl sm:text-3xl shadow-xl mt-8 sm:mt-12 italic shadow-orange-500/20">Verify Node</button>
          </div>
        )}
        {step === 3 && (
          <div className="space-y-8 sm:space-y-12 animate-enter">
            <div className="text-center"><h2 className="text-4xl sm:text-6xl font-black text-[#1b4332] italic">Trade Profile</h2></div>
            <div className="space-y-4 sm:space-y-6">
              <input className="w-full px-8 py-6 sm:py-8 bg-stone-50 rounded-[24px] sm:rounded-[40px] font-black text-xl sm:text-2xl outline-none shadow-inner" placeholder="Legal Name / Entity" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
              <input className="w-full px-8 py-6 sm:py-8 bg-stone-50 rounded-[24px] sm:rounded-[40px] font-black text-xl sm:text-2xl outline-none shadow-inner" placeholder="Operations Base (District)" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} />
              <button onClick={() => setStep(4)} className="w-full py-6 sm:py-8 bg-[#1b4332] text-white rounded-[24px] sm:rounded-[40px] font-black text-xl sm:text-3xl shadow-xl mt-6 italic">Secure Clearance</button>
            </div>
          </div>
        )}
        {step === 4 && (
          <div className="space-y-8 sm:space-y-16 animate-enter">
            <div className="bg-[#1b4332] p-8 sm:p-20 rounded-[32px] sm:rounded-[64px] text-white relative shadow-2xl border border-white/5 overflow-hidden text-center">
              <p className="text-[10px] sm:text-[14px] font-black text-white/30 uppercase tracking-[0.5em] sm:tracking-[0.7em] mb-4 sm:mb-8">Node Activation Fee</p>
              <h2 className="text-[60px] sm:text-[100px] font-black italic tracking-tighter mb-6 sm:mb-10 leading-none">₹{currentFee}</h2>
            </div>
            <button onClick={() => login(phone, role, { name: formData.name, location: formData.location })} className="w-full py-6 sm:py-8 bg-[#ff9f1c] text-white rounded-[24px] sm:rounded-[40px] font-black text-2xl sm:text-4xl shadow-xl italic shadow-orange-500/30">Activate Trading Node</button>
          </div>
        )}
      </div>
    </div>
    ...
    <Layout activeTab={activeTab} setActiveTab={setActiveTab}>
      <div className="animate-enter">
        {activeTab === 'dashboard' ? <FarmerDashboard /> : 
         activeTab === 'browse' ? <MarketBrowser /> : 
         activeTab === 'orders' ? <BuyerOrders /> : 
         activeTab === 'admin' ? <AdminDashboard /> : 
         activeTab === 'add' ? <AddProductForm onComplete={() => setActiveTab('dashboard')} /> : 
         null}
      </div>
    </Layout>
  );
